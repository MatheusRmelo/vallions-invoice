...
const CompetenceConferenceForm = ({ open, onClose, price, unity, conference, startAt, endAt }: Props) => {
    ...
    const init = () => {
        ...
        var end = endAt;
        setMonths(
            dataMonths.filter((element) => Number(element.number) >= start.getMonth() + 1 && Number(element.number) <= end.getMonth() + 1)
        );
        setMonth('');
    };
    ...
    return (
        <Dialog fullWidth maxWidth={'md'} open={open} onClose={() => onClose(false)}>
            <Box margin={'10px'}>
                <DialogTitle>
                    <span style={{ fontSize: '3vh', fontWeight: 'bold' }}>Competência de Faturamento</span>
                </DialogTitle>
                <DialogContent>
                    <DialogContentText style={{ fontSize: '1.3vh' }}></DialogContentText>
                    <Box height={40} />
                        ...
                        <Grid item xs={6}>
                            <FormControl fullWidth>
                                ...
                                <Select labelId="select-label" label="Select" value={month} onChange={(e) => setMonth(e.target.value)}>
                                    {months.map((element) => (
                                        <MenuItem value={element.number}>{element.label}</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                        </Grid>
